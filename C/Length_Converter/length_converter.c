...
int main(){
	...
	do{
		...
		if (userInput == '1'){
			...
			/* Variables we will use our formula on */
			float centimetreConverted = 0;
			float feetInput = 0;
			/* Variables that are used for error checking to see if input is a number or not */
			char input[100] = "";
			int result = 0;
   
			printf("*** Feet to Centimetre (ft to cm) ***\n");
			printf("Please enter Feet value: \n");
			scanf ("%s", input);
			
			/* Check if the input is acceptable */
			result = checkIsNumber(input);
			if (result == EXIT_SUCCESS){
				feetInput = (float)atof(input);

				/* Convert from Feet to Centimetre */
				centimetreConverted = feetInput / 0.032808;
				printf("Centimetre result: %f\n", centimetreConverted);
...
void menu(){
	...
	printf("1) Centimetre to Feet (cm to ft)\n");
	printf("2) Feet to Centimetre (ft to cm)\n");
